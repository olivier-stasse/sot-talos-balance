...
#include "sot/talos_balance/admittance-controller-op-point.hh"
...
#define PROFILE_ADMITTANCECONTROLLEROPPOINT_WFORCE_COMPUTATION \
  "AdmittanceControllerOpPoint: w_force computation   "

#define PROFILE_ADMITTANCECONTROLLEROPPOINT_WDQ_COMPUTATION \
  "AdmittanceControllerOpPoint: w_dq computation      "

#define PROFILE_ADMITTANCECONTROLLEROPPOINT_DQ_COMPUTATION \
  "AdmittanceControllerOpPoint: dq computation        "
...
typedef AdmittanceControllerOpPoint EntityClassName;

/* --- DG FACTORY ---------------------------------------------------- */
DYNAMICGRAPH_FACTORY_ENTITY_PLUGIN(AdmittanceControllerOpPoint,
                                   "AdmittanceControllerOpPoint");

/* ------------------------------------------------------------------- */
/* --- CONSTRUCTION -------------------------------------------------- */
/* ------------------------------------------------------------------- */
AdmittanceControllerOpPoint::AdmittanceControllerOpPoint(const std::string &name)
    : Entity(name),
      ...
      m_initSucceeded(false)
{
  ...
  addCommand("init", makeCommandVoid1(*this,
                                       &AdmittanceControllerOpPoint::init,
                                       docCommandVoid1("Initialize the entity.",
                                                      "time step")));
  addCommand("resetDq", makeCommandVoid0(*this,
    &AdmittanceControllerOpPoint::resetDq,
    docCommandVoid0("resetDq")));
}

void AdmittanceControllerOpPoint::init(const double &dt)
{
...
void AdmittanceControllerOpPoint::resetDq()
{
  m_w_dq.setZero(m_n);
  return;
}
...
DEFINE_SIGNAL_INNER_FUNCTION(w_force, dynamicgraph::Vector)
{
  ...
  getProfiler().start(PROFILE_ADMITTANCECONTROLLEROPPOINT_WFORCE_COMPUTATION);
  ...
  getProfiler().stop(PROFILE_ADMITTANCECONTROLLEROPPOINT_WFORCE_COMPUTATION);
...
DEFINE_SIGNAL_INNER_FUNCTION(w_dq, dynamicgraph::Vector)
{
  ...
  getProfiler().start(PROFILE_ADMITTANCECONTROLLEROPPOINT_WDQ_COMPUTATION);
  ...
  getProfiler().stop(PROFILE_ADMITTANCECONTROLLEROPPOINT_WDQ_COMPUTATION);
...
DEFINE_SIGNAL_OUT_FUNCTION(dq, dynamicgraph::Vector)
{
  ...
  getProfiler().start(PROFILE_ADMITTANCECONTROLLEROPPOINT_DQ_COMPUTATION);
  ...
  getProfiler().stop(PROFILE_ADMITTANCECONTROLLEROPPOINT_DQ_COMPUTATION);
...
/* --- ENTITY -------------------------------------------------------- */
/* ------------------------------------------------------------------- */
void AdmittanceControllerOpPoint::display(std::ostream &os) const
{
  os << "AdmittanceControllerOpPoint " << getName();
  try
  {
