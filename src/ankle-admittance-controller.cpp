...
#define OUTPUT_SIGNALS m_dRPSOUT
          ...
          , m_initSucceeded(false)
      {
        ...
        /* Commands. */
        addCommand("init", makeCommandVoid0(*this, &AnkleAdmittanceController::init, docCommandVoid0("Initialize the entity.")));
      }

      void AnkleAdmittanceController::init()
      {;
        if(!m_gainsXYSIN.isPlugged())
          return SEND_MSG("Init failed: signal gainsXY is not plugged", MSG_TYPE_ERROR);
        if(!m_wrenchSIN.isPlugged())
          return SEND_MSG("Init failed: signal wrench is not plugged", MSG_TYPE_ERROR);
