...
inline double wEulerMean(double a1, double a2, double w1, double w2) {
  ...
  if (a1 - a2 >= EIGEN_PI)
    return (EIGEN_PI * (w1 - w2) / (w2 + w1) - wMean) < 0 ? -EIGEN_PI + wMean - EIGEN_PI * (w1 - w2) / (w2 + w1)
                                                          : EIGEN_PI + wMean - EIGEN_PI * (w1 - w2) / (w2 + w1);
  else if (a1 - a2 < -EIGEN_PI)
    return (EIGEN_PI * (w2 - w1) / (w1 + w2) - wMean) < 0 ? -EIGEN_PI + wMean - EIGEN_PI * (w2 - w1) / (w1 + w2)
...
#define OUTPUT_SIGNALS                                                                                              \
  m_qSOUT << m_vSOUT << m_q_lfSOUT << m_q_rfSOUT << m_q_imuSOUT << m_w_lfSOUT << m_w_rfSOUT << m_w_lf_filteredSOUT  \
          << m_w_rf_filteredSOUT << m_lf_xyzquatSOUT << m_rf_xyzquatSOUT << m_v_acSOUT << m_a_acSOUT << m_v_kinSOUT \
          << m_v_imuSOUT << m_v_gyrSOUT << m_v_flexSOUT
...
TalosBaseEstimator::TalosBaseEstimator(const std::string& name)
             ...
             makeCommandVoid1(
                 *this, &TalosBaseEstimator::set_alpha_DC_vel,
                 docCommandVoid1("Set the filter parameter for removing DC from velocity integrated from acceleration",
                                 "double")));
  addCommand("reset_foot_positions", makeCommandVoid0(*this, &TalosBaseEstimator::reset_foot_positions,
  ...
  addCommand(
      ...
      makeCommandVoid1(
          *this, &TalosBaseEstimator::set_zmp_std_dev_right_foot,
  ...
  addCommand(
      ...
      makeCommandVoid1(
          *this, &TalosBaseEstimator::set_zmp_std_dev_left_foot,
...
void TalosBaseEstimator::set_zmp_margin_right_foot(const double& margin) { m_zmp_margin_rf = margin; }

void TalosBaseEstimator::set_zmp_margin_left_foot(const double& margin) { m_zmp_margin_lf = margin; }

void TalosBaseEstimator::set_normal_force_margin_right_foot(const double& margin) { m_fz_margin_rf = margin; }

void TalosBaseEstimator::set_normal_force_margin_left_foot(const double& margin) { m_fz_margin_lf = margin; }
...
double TalosBaseEstimator::compute_zmp_weight(const Vector2& zmp, const Vector4& foot_sizes, double std_dev,
                                              double margin) {
...
void TalosBaseEstimator::reset_foot_positions_impl(const Vector6& ftlf, const Vector6& ftrf) {
  ...
  kinematics_estimation(ftrf, m_K_rf, m_oMrfs, m_right_foot_id, rfMff, dummy,
                        dummy1);  // rfMff is obtained reading oMff becaused oMrfs is here set to Identity
...
void TalosBaseEstimator::kinematics_estimation(const Vector6& ft, const Vector6& K, const SE3& oMfs,
...
DEFINE_SIGNAL_OUT_FUNCTION(q, dynamicgraph::Vector) {
  ...
  if (s.size() != static_cast<Eigen::Index>(m_robot_util->m_nbJoints + 6)) s.resize(m_robot_util->m_nbJoints + 6);
  ...
  getProfiler().start(PROFILE_BASE_POSITION_ESTIMATION);
  {
    ...
    // get rpy
    const SE3 ffMtorso(
        m_data->oMi[m_torso_id]);            // transform between freeflyer and body attached to IMU sensor (torso)
    ...
    Vector3 rpy_torso, rpy_torso_lf, rpy_torso_rf,
        rpy_torso_imu;  // orientation of the body which imu is attached to. (fusion, from left kine, from right kine,
                        // from imu)
    ...
    if (m_K_fb_feet_posesSIN.isPlugged()) {
      ...
      if (K_fb > 0.0 && m_w_imu > 0.0) {
        ...
        if (m_lf_ref_xyzquatSIN.isPlugged() and m_rf_ref_xyzquatSIN.isPlugged()) {
          ...
          const Eigen::Quaterniond ql(m_lf_ref_xyzquatSIN(iter)(6), m_lf_ref_xyzquatSIN(iter)(3),
                                      m_lf_ref_xyzquatSIN(iter)(4), m_lf_ref_xyzquatSIN(iter)(5));
          const Eigen::Quaterniond qr(m_rf_ref_xyzquatSIN(iter)(6), m_rf_ref_xyzquatSIN(iter)(3),
...
DEFINE_SIGNAL_OUT_FUNCTION(v, dynamicgraph::Vector) {
  ...
  getProfiler().start(PROFILE_BASE_VELOCITY_ESTIMATION);
  {
    ...
    // we make the assumption that we are 'turning around the foot' with pure angular velocity in the ankle measured by
    // the gyro
    ...
    // const SE3 torsoMimu(Matrix3::Identity(), Vector3(-0.13, 0.0,  0.118)); ///TODO Read this transform from setable
    // parameter /// TODO chesk the sign of the translation const SE3 ffMtorso(m_data->oMf[m_torso_id]); const SE3
    // imuMff = (ffMtorso * m_torsoMimu).inverse(); gVw_a =  gVo_g + gHa.act(aVb_a)-gVb_g //angular velocity in the
    // ankle from gyro and d_enc const Frame & f_imu = m_model.frames[m_torso_id]; const Frame & f_imu =
    // m_model.frames[m_IMU_frame_id];
    Vector3 gVo_a_l =
        Vector3(gyr_imu(0), gyr_imu(1),
                gyr_imu(2));  //+ (imuMff*ffMlf).act(v_lf_local).angular() - m_data->v[f_imu.parent].angular();
    Vector3 gVo_a_r =
        Vector3(gyr_imu(0), gyr_imu(1),
                gyr_imu(2));  //+ (imuMff*ffMrf).act(v_rf_local).angular() - m_data->v[f_imu.parent].angular();
    ...
    // const SE3 torsoMimu(Matrix3::Identity(), +1.0*Vector3(-0.13, 0.0,  0.118)); ///TODO Read this transform from
