...
void CRTPacket::SetData(char *ptr) {
  ...
  if (GetType() == PacketData) {
    ...
    for (nComponent = 1; nComponent <= mnComponentCount && nComponentType > 0 && nComponentType < ComponentNone;
         nComponent++) {
      ...
      if (nComponentType == ComponentImage) {
        ...
        for (nCamera = 1; nCamera < mnImageCameraCount; nCamera++) {
          mpImageData[nCamera] =
              mpImageData[nCamera - 1] + 36 + SetByteOrder((unsigned int *)(mpImageData[nCamera - 1] + 32));
        }
      }
      ...
      if (nComponentType == ComponentForce) {
        ...
        for (nDevice = 1; nDevice < mnForcePlateCount; nDevice++) {
          if ((mnMajorVersion == 1) && (mnMinorVersion == 0)) {
            ...
            mpForceData[nDevice] =
                mpForceData[nDevice - 1] + 12 + SetByteOrder((unsigned int *)(mpForceData[nDevice - 1] + 4)) * 36;
          }
        }
      }
      ...
      if (nComponentType == ComponentGazeVector) {
        ...
        for (nDevice = 1; nDevice < mnGazeVectorCount; nDevice++) {
          ...
          mpGazeVectorData[nDevice] =
              mpGazeVectorData[nDevice - 1] + 4 + ((nPrevSampleCount == 0) ? 0 : 4) + nPrevSampleCount * 24;
        }
      }
...
unsigned long long CRTPacket::GetTimeStamp(char *pData, bool bBigEndian) {
  if (GetType(pData, bBigEndian) == PacketData) {
    if (bBigEndian) {
      return ((unsigned long long)(ntohl((long)*((long long *)(pData + 8)))) << 32) +
...
bool CRTPacket::Get2DLinMarker(unsigned int nCameraIndex, unsigned int nMarkerIndex, unsigned int &nX,
...
bool CRTPacket::Get6DOFResidualBody(unsigned int nBodyIndex, float &fX, float &fY, float &fZ, float afRotMatrix[9],
                                    float &fResidual) {
  char *pData = mpComponentData[Component6dRes - 1];
...
bool CRTPacket::Get6DOFEulerBody(unsigned int nBodyIndex, float &fX, float &fY, float &fZ, float &fAng1, float &fAng2,
                                 float &fAng3) {
  char *pData = mpComponentData[Component6dEuler - 1];
...
bool CRTPacket::Get6DOFEulerResidualBody(unsigned int nBodyIndex, float &fX, float &fY, float &fZ, float &fAng1,
                                         float &fAng2, float &fAng3, float &fResidual) {
  char *pData = mpComponentData[Component6dEulerRes - 1];
...
bool CRTPacket::GetTimecodeIRIG(unsigned int nTimecodeIndex, int &year, int &day, int &hours, int &minutes,
...
bool CRTPacket::GetImageCrop(unsigned int nCameraIndex, float &fCropLeft, float &fCropTop, float &fCropRight,
                             float &fCropBottom) {
  if (mnImageCameraCount <= nCameraIndex) {
    return false;
  }
...
unsigned int CRTPacket::GetAnalogData(unsigned int nDeviceIndex, unsigned int nChannelIndex, float *pDataBuf,
...
bool CRTPacket::GetAnalogData(unsigned int nDeviceIndex, unsigned int nChannelIndex, unsigned int nSampleIndex,
  ...
  if (nDeviceIndex < mnAnalogDeviceCount) {
    ...
    if (GetAnalogChannelCount(nDeviceIndex) > nChannelIndex && nSampleCount > nSampleIndex) {
      if ((mnMajorVersion == 1) && (mnMinorVersion == 0)) {
...
//                       Analog Single
//-----------------------------------------------------------
unsigned int CRTPacket::GetAnalogSingleDeviceCount() { return mnAnalogSingleDeviceCount; }
    ...
    if ((mnMajorVersion == 1) && (mnMinorVersion == 0)) {
      if (nPlateIndex == 0) {
        for (unsigned int k = 0; k < 9; k++) {
          *(((float *)pForceBuf) + k) = (float)SetByteOrder((double *)(mpForceData[nPlateIndex] + k * sizeof(double)));
        }
        nSize = 1;
      }
    } else {
  ...
  if (nPlateIndex < mnForcePlateCount) {
    if ((mnMajorVersion == 1) && (mnMinorVersion == 0)) {
      if (nPlateIndex == 0 && nForceIndex == 0) {
        for (unsigned int k = 0; k < 9; k++) {
          *(((float *)&sForce) + k) = (float)SetByteOrder((double *)(mpForceData[nPlateIndex] + k * sizeof(double)));
      ...
      if (nForceIndex < GetForceCount(nPlateIndex)) {
        for (unsigned int k = 0; k < 9; k++) {
          *(((float *)&sForce) + k) = SetByteOrder(
              (float *)(mpForceData[nPlateIndex] + 12 + k * sizeof(float) + nForceIndex * sizeof(SForce)));
...
bool CRTPacket::GetSkeletonSegments(unsigned int nSkeletonIndex, SSkeletonSegment *segmentBuffer,
                                    unsigned int nBufSize) {
  if (mSkeletonCount <= nSkeletonIndex) {
    return false;
  }
...
//                       Force Single
//-----------------------------------------------------------
unsigned int CRTPacket::GetForceSinglePlateCount() { return mnForceSinglePlateCount; }
...
bool CRTPacket::GetForceSingleData(unsigned int nPlateIndex, SForce &sForce) {
  if (nPlateIndex < mnForceSinglePlateCount) {
    for (unsigned int k = 0; k < 9; k++) {
      *(((float *)&sForce) + k) = SetByteOrder((float *)(mpForceSingleData[nPlateIndex] + 4 + k * sizeof(float)));
...
double CRTPacket::SetByteOrder(double *pfData) {
  ...
  if (mbBigEndian) {
    nTmp = (((unsigned long long)(ntohl((long)*((unsigned long long *)pfData))) << 32) +
