...
#define OUTPUT_SIGNALS
      ...
      QualisysClient(const std::string& name)
        : Entity(name)
        ,CONSTRUCT_SIGNAL_IN(dummy,  double)
      ...
      void QualisysClient::registerRigidBody(const std::string& RBname)
      {
       ...
       sig = new dg::SignalTimeDependent< dg::Vector,int >(boost::bind(&QualisysClient::readGenericRigidBody,this,RBidx,_1,_2),m_dummySIN,getClassName()+"("+getName()+")::output(dynamicgraph::Vector)::xyzquat_"+RBname);
      ...
      void QualisysClient::getRigidBodyList()
      {
        m_printRigidBodyList = true;
      }
